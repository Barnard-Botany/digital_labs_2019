...
```{r}
#These examples from the genus Jungia were for the Midterm exam in April 2019
#J_rugosa<-BIEN_occurrence_species(species = "Jungia rugosa")
#J_spectabilis<-BIEN_occurrence_species(species = "Jungia spectabilis")
#J_columbiana<-BIEN_occurrence_species(species = "Jungia columbiana")
...
```{r}
#These examples from the genus Jungia were used for the Midterm exam in April 2019
#J_rugosa_rangemap <- BIEN_ranges_load_species("Jungia rugosa")
#J_spectabilis_rangemap<- BIEN_ranges_load_species("Jungia spectabilis")
...
Mapping occurrence data is informative for discering species ranges overlap. But could we add  more information, such as how each species overlaps with climate variables?. 
...
3. Make a South America map with occurrence data for Juncia pauciflora & J. polita and:
...
```{r}
#if you just one exactly one taxonomic rank lower, then this is referred to as "children"
...
And, beyond gbif, there are other database-specific packages like "rebird" for the Citizen Science app and database E-bird and "rinat" for working with I-Naturalist. 
...
And, generally, data from such databases needs to be "cleaned" in a number of ways, for which a package called "scrubr" These packages for digital natural history and biodiversity science were created and are maintained by Scott Chamberlain, who works for R-OpenSci.
...
```{r}
#Or, we could use a loop, which will re-iteratres the same command over and over again but each time changing the command
...
#An example loop is in the chunk below, using the vector RecordTypes, define above
...
Let's go back to the our peninsula, which is a hotspot of endemism for many plant and animal species Remember to load the R Package "rgbif"
...
Very good habit to trim the default data downloaded to be smaller, and make sure it's a data frame. Also make sure you have onlyl complete cases because some of the code below with choke if there are missing values. Also, sometimes the same individual plant/population will be recorded at multiples times in the same location, which is why you may want to run the "unique" commands. The "dim" command helps you look at how you trimmed down the file.
...
This package is great for working GBIF or other databases. 
...
#Here, to speed things up during class, I've further limited to just 50 recordss
...
#It's nifty that the package "spocc" convert this huge result into a slimmed down dataframe
...
Going back to the our peninsula, here are a bunch of species that are endemic to that area.
...
2. Look up if the two plant taxa. Are they both angiosperms?
